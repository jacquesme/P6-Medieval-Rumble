...
var gun1 = new Weapon('Gun1', 25, 'wp-1 weapon');
var sword1 = new Weapon('Sword1', 18, 'wp-2 weapon');
var sword2 = new Weapon('Sword2', 16, 'wp-3 weapon');
var cupcake = new Weapon('CupCake', 10, 'wp-4 weapon');
var cake = new Weapon('Cake', 12, 'wp-5 weapon');
var gun2 = new Weapon('Gun2', 20, 'wp-6 weapon');
var extra = new Weapon('Extra1', 10, 'quiz-1');
...
function fight(posNew, posOld) {
    ...
    if (posNew.x === posOld.x && posNew.y <= posOld.y + 1 && posNew.y >= posOld.y - 1 || posNew.y === posOld.y && posNew.x <= posOld.x + 1 && posNew.x >= posOld.x - 1) {
        move = false;
        hover = false;
...
var boxPosition = function boxPosition(itemClass) {
    return $(itemClass).data('index');
};
var playerPosition = boxPosition('.player1');
// Old position is the position of not active player in the moment
var posOld = getCoordinates(playerPosition);
